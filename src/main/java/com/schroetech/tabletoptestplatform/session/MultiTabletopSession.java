...
public class MultiTabletopSession extends AbstractTabletopSession {
    ...
    public void playGames() {
        ...
        for (int i = 0; i < getNumberOfGames(); i++) {
            SingleTabletopSession singleSession = new SingleTabletopSession();
            singleSession.setGame(this.getGame());
            singleSession.addPlayers(this.getPlayers());

            IPlayer winner = singleSession.playGame(false);
            if (winner != null) {
